...
import { updateLocation } from "@/lib/actions/admin.location.actions";
import { Button } from "@/components/ui/button";
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage } from "@/components/ui/form";
import { Input } from "@/components/ui/input";

export default function AdminEditLocationForm({ initialData }: { initialData: Location }) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    const result = await updateLocation(initialData.id, values);
      ...
      <form method="POST" onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 py-6">
        ...
        <div className="flex items-center justify-end space-x-2">
          ...
          <Button size="sm" type="submit" disabled={form.formState.isSubmitting}>
            {form.formState.isSubmitting ? "Editing..." : "Edit Location"}
          </Button>
        </div>
      </form>
    </Form>
